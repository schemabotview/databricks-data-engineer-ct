...
- **Modules 04 & 05 — authored** (11 + 10 sections): `notebooks/` split from the reference `04-transformations.ipynb` / `05-modeling-pipelines.ipynb` (hands-on + self-check scaffolding dropped), `slides/` + `tts/` written, `manifest.json` wired (all riding the shared `databricks-data-engineer` scene; §01 of each is the `hook`). Module 04 frames on `engine`/`spark-engine` (bookended by `medallion` bronze/silver); module 05 frames on `medallion` gold + the `declarative-pipelines` children (`dlt-table`, `streaming-table`, `dlt-expectations`, `apply-changes`). Pending: generate `audio/04-*.wav` and `audio/05-*.wav` from the `.tts` via Colab.
- **Modules 06–09** — not started (section lists agreed in `README.md`; repeat the module-01 pipeline).

Next: generate module 04 + 05 audio (Colab), then author module 06 (`notebook → .tts → .slide` per section, wire the manifest).
