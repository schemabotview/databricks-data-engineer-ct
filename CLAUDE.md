...
- **Modules 04, 05, 06 & 07 — authored** (11 + 10 + 10 + 9 sections): `notebooks/` split from the reference `04-transformations.ipynb` / `05-modeling-pipelines.ipynb` / `06-lakeflow-jobs.ipynb` / `07-cicd-bundles.ipynb` (hands-on + self-check scaffolding dropped), `slides/` + `tts/` written, `manifest.json` wired (all riding the shared `databricks-data-engineer` scene; §01 of each is the `hook`). Module 04 frames on `engine`/`spark-engine` (bookended by `medallion` bronze/silver); module 05 on `medallion` gold + the `declarative-pipelines` children; module 06 on the `lakeflow-jobs` cluster (`job-dag`/`job-triggers` children, `compute`); module 07 on the `cicd` cluster — `repos`, `databricks-yml`, `databricks-cli`, `gh-actions`, `run-as-sp`. Pending: generate `audio/04-*.wav` … `audio/07-*.wav` from the `.tts` via Colab.
- **Modules 08–09** — not started (section lists agreed in `README.md`; repeat the module-01 pipeline).

Next: generate module 04–07 audio (Colab), then author module 08 (`notebook → .tts → .slide` per section, wire the manifest).
