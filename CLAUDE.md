...
- **Module 04 — authored** (11 sections): `notebooks/04-*.ipynb` split from the reference `04-transformations.ipynb` (the hands-on + self-check scaffolding dropped), `slides/04-*.slide` + `tts/04-*.tts` written, and `manifest.json` wired — §01 hook framed on `medallion` (`bronze`/`silver`), §02–10 on the `engine`/`spark-engine` transformation nodes, §11 back on `silver` (all riding the shared `databricks-data-engineer` scene). Pending: generate `audio/04-*.wav` from the `.tts` via Colab.
- **Modules 05–09** — not started (section lists agreed in `README.md`; repeat the module-01 pipeline).

Next: generate module 04 audio (Colab), then author module 05 (`notebook → .tts → .slide` per section, wire the manifest).
