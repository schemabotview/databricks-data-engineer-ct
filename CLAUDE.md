...
- **Modules 02 & 03 — complete end-to-end**: `notebooks/` split from the reference per-module notebooks, `slides/` + `tts/` written, `audio/*.wav` generated via Colab, and `manifest.json` wired (all riding the shared `databricks-data-engineer` scene; §01 of each is the `hook`). Module 02 = 10 sections (Delta/UC nodes); module 03 = 11 sections (ingestion nodes).
- **Modules 04–09 — authored** (11 + 10 + 10 + 9 + 10 + 9 sections): `notebooks/` split from the reference `04-transformations.ipynb` / `05-modeling-pipelines.ipynb` / `06-lakeflow-jobs.ipynb` / `07-cicd-bundles.ipynb` / `08-troubleshooting-optimization.ipynb` / `09-governance-security.ipynb` (hands-on + self-check scaffolding dropped; module 09 also drops managed/external + converting, already taught in module 02), `slides/` + `tts/` written, `manifest.json` wired (all riding the shared `databricks-data-engineer` scene; §01 of each is the `hook`). Module 04 frames on `engine`/`spark-engine` (bookended by `medallion` bronze/silver); module 05 on `medallion` gold + the `declarative-pipelines` children; module 06 on the `lakeflow-jobs` cluster; module 07 on the `cicd` cluster; module 08 on `lakeflow-jobs`→`engine`/`spark-engine`/`delta-lake`/`compute`; module 09 on the `unity-catalog` cluster (`uc-metastore`, `uc-namespace`, `uc-grant`, `uc-lineage`). Pending: generate `audio/04-*.wav` … `audio/09-*.wav` from the `.tts` via Colab.

**All 9 modules (90 sections) are now authored and manifest-wired** — the full course spine is complete. The only remaining work is generating `audio/*.wav` for modules 04–09 via `scripts/colab_generate_audio.ipynb` (modules 01–03 audio already generated).

Next: generate module 04–09 audio (Colab) — then the course is recordable end-to-end.
