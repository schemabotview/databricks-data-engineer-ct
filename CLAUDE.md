...
- **Modules 04–08 — authored** (11 + 10 + 10 + 9 + 10 sections): `notebooks/` split from the reference `04-transformations.ipynb` / `05-modeling-pipelines.ipynb` / `06-lakeflow-jobs.ipynb` / `07-cicd-bundles.ipynb` / `08-troubleshooting-optimization.ipynb` (hands-on + self-check scaffolding dropped), `slides/` + `tts/` written, `manifest.json` wired (all riding the shared `databricks-data-engineer` scene; §01 of each is the `hook`). Module 04 frames on `engine`/`spark-engine` (bookended by `medallion` bronze/silver); module 05 on `medallion` gold + the `declarative-pipelines` children; module 06 on the `lakeflow-jobs` cluster; module 07 on the `cicd` cluster; module 08 on `lakeflow-jobs`→`engine`/`spark-engine` (diagnostics/perf), `delta-lake`/`optimize-zorder` (maintenance), `compute`/`job-cluster`/`serverless` (failures). Pending: generate `audio/04-*.wav` … `audio/08-*.wav` from the `.tts` via Colab.
- **Module 09** — not started (section list agreed in `README.md`; repeat the module-01 pipeline).

Next: generate module 04–08 audio (Colab), then author module 09 — the last (`notebook → .tts → .slide` per section, wire the manifest).
